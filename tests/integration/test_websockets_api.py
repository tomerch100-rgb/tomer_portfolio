...
def test_websocket_cswsh_protection_rejects_unauthorized_origin(sync_client: TestClient, test_user: User):
    ...
    with (
        ...
        sync_client.websocket_connect(f"/ws?token={token}", headers={"Origin": "https://malicious-untrusted-site.com"}),
    ):
        pass
...
def test_websocket_multi_tab_broadcast_to_same_user(sync_client: TestClient, test_user: User):
    ...
    with (
        sync_client.websocket_connect(f"/ws?token={token}", headers=headers) as tab1,
        sync_client.websocket_connect(f"/ws?token={token}", headers=headers) as tab2,
    ):
