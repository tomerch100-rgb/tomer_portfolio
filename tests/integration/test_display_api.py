...
@pytest.mark.asyncio
async def test_show_portfolio_returns_holdings(
    ...
    stock = Portfolio(user_id=test_user.user_id, ticker="MSFT", shares=10, avg_price=300.00, sector="Technology")
...
@pytest.mark.asyncio
async def test_portfolio_summary_calculation(
    client: AsyncClient, auth_headers: dict, test_user: User, db_session: Session
):
...
async def test_update_position_tp_sl(client: AsyncClient, auth_headers: dict, test_user: User, db_session: Session):
    ...
    stock = Portfolio(user_id=test_user.user_id, ticker="TSLA", shares=10, avg_price=200.00, sector="Consumer Cyclical")
    db_session.add(stock)
    db_session.commit()

    payload = {"ticker": "TSLA", "take_profit": 280.00, "stop_loss": 175.00, "risk_level": "Medium"}
