...
async def test_parse_valid_excel_xlsx():
    ...
    df = pd.DataFrame({"Symbol": ["AMZN", "GOOGL"], "Quantity": [12, 20], "Price": [180.0, 165.5]})
    output = io.BytesIO()
...
async def test_preview_row_limit():
    ...
    _, preview_rows, total = await excel_parser.parse_preview(csv_bytes, "large.csv", max_preview_rows=5)
    assert total == 50
