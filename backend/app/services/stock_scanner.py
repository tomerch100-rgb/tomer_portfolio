    ...
    try:
        def _get_active_records():
            ...
            portfolio_alerts = db.scalars(
                ...
                .where(
                    or_(
                        (Portfolio.take_profit.isnot(None)) & (Portfolio.tp_triggered == False),
                        (Portfolio.stop_loss.isnot(None)) & (Portfolio.sl_triggered == False)
                    )
                )
            ).all()
        ...
        for ticker, p_data in prices_map.items():
            if isinstance(p_data, dict) and p_data.get("lastPrice") is not None:
                ...
                price_broadcast_tasks.append(manager.broadcast({
                    "type": "PRICE_UPDATE",
                    "symbol": ticker,
                    "ticker": ticker,
                    "price": round(lp, 2),
        ...
        if alert_tasks or ws_tasks:
            ...
            logger.info(f"✅ עודכנו {len(triggered_watchlist)} התראות Watchlist ו-{len(triggered_portfolio_tp) + len(triggered_portfolio_sl)} התראות Portfolio במסד הנתונים.")
