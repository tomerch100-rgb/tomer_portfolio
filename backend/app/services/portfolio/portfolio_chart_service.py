...
async def get_protfolio_pie(db: Session, user_id: int):
    ...
    for stock in info_st:
        ...
        stock_data = {"ticker": stock.ticker, "value": info["stock_currnet_worth"]}
        included_li.append(stock_data)
    return included_li


async def get_daily_change(db: Session, user_id: int):
    info_st = crud_portfolio.get_portfolio_all(db, user_id)
    daily_change = []
    for stock in info_st:
        info = await hp.update_prices(stock.ticker, stock.shares, stock.avg_price)
