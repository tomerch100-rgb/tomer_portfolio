...
router = APIRouter(prefix="/api/portfolio/import", tags=["portfolio-import"])
...
async def preview_portfolio_import(
    ...
    try:
        preview_response = await import_service.preview_import(file_bytes=file_bytes, filename=file.filename)
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Failed to parse file: {e!s}")
    except Exception as e:
        logger.exception(f"Unexpected error during import preview: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Internal error processing import preview: {e!s}"
        )
...
async def confirm_portfolio_import(
    payload: ImportConfirmRequest, user_id: int = Depends(security.get_current_user_id), db: Session = Depends(get_db)
):
    """
    ...
    if not payload.mapping:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Column mapping dictionary is required.")
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Internal error during import confirmation: {e!s}",
        )
