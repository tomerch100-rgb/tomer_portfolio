...
def run_tests():
    ...
    login_payload = {"username": "test_telegram_qa_user", "password": "testpass"}
    ...
    webhook_payload = {
        ...
        "message": {
            "message_id": 999,
            "from": {"id": 987654321, "is_bot": False, "first_name": "QA_Tester", "username": "qa_tester"},
            "chat": {"id": 987654321, "type": "private", "first_name": "QA_Tester"},
            "date": 1600000000,
            "text": f"/start {token}",
        },
    }
    ...
    try:
        ...
        if user.telegram_connect_token is not None:
            logger.error(
                f"❌ DB linking failed: user.telegram_connect_token is still {user.telegram_connect_token}, expected None"
            )
            return False
        logger.info(
            "✅ Database check passed: User is successfully linked (Telegram ID stored, connect token cleared)."
        )
    finally:
        db.close()
